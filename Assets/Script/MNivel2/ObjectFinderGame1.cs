...
public class ObjectFinderGame1 : MonoBehaviour
{
    [System.Serializable]
    public class ObjectQuizData
    {
        ...
        public Button continueButton; //  NUEVO botón de continuar
    }
    ...
    void Start()
    {
        foreach (var quiz in objectQuizzes)
        {
            ...
            quiz.continueButton.gameObject.SetActive(false); // Desactiva el botón al inicio
            quiz.continueButton.onClick.AddListener(() => ContinueAfterWin(quiz)); // Escucha el clic
            SetupButtons(quiz);
        }
    ...
    IEnumerator WaitForQuestionEnd()
{
    ...
    void CorrectAnswer(ObjectQuizData quiz, Button clickedButton)
    {
        ...
        if (correctAnswersGiven[quiz] >= 2)
        {
            ...
            quiz.continueButton.gameObject.SetActive(true); // ✨ Muestra botón de continuar
        }
    }
